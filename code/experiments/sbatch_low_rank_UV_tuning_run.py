...
def cmd(tuning_csv_file, base_dir, loo_op_dir, std_op_dir, incr, timeout, memory_gb):
    ...
    for context_id, seed, rank, alpha, sigma, i_loo, j_loo in df[['context_id', 'seed', 'rank', 'alpha', 'sigma', 'i_loo', 'j_loo']].values:
        ...
        cmd = f'sbatch --mem={memory} --time={timeout} -o "{op_std_file}" ./sbatch_low_rank_UV_tuning_job.sh {in_mat_file} {seed} {rank} {alpha} {sigma} {i_loo} {j_loo} {context_id} "{op_loo_file}"'
        # print(cmd)
        os.system(cmd)
